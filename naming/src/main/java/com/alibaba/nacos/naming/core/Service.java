...
public class Service extends com.alibaba.nacos.api.naming.pojo.Service implements Record, RecordListener<Instances> {
    ...
    public void updateIPs(Collection<Instance> instances, boolean ephemeral) {
        ...
        for (Instance instance : instances) {
            try {
                ...
                //instance对应的cluster不存在时，创建一个并放入clusterMap中
        ...
        for (Map.Entry<String, List<Instance>> entry : ipMap.entrySet()) {
            ...
            //将instance放入对应的cluster的ephemeralInstances或persistentInstances
            //根据ephemeral决定
            clusterMap.get(entry.getKey()).updateIps(entryIPs, ephemeral);
        }

        setLastModifiedMillis(System.currentTimeMillis());
        //发布service改变事件，通过udp推送订阅者service发生改变
    ...
    public List<Instance> allIPs(boolean ephemeral) {
        List<Instance> result = new ArrayList<>();
        //遍历service所属的cluster，根据ephemeral，
        //将ephemeralInstances、persistentInstances信息放入result
        for (Map.Entry<String, Cluster> entry : clusterMap.entrySet()) {
