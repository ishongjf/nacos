...
public class ServiceManager implements RecordListener<Service> {
    ...
    public void createServiceIfAbsent(String namespaceId, String serviceName, boolean local, Cluster cluster)
        ...
        //根据namespaceId和serviceName从serviceMap中获取service，不存在时创建service
        Service service = getService(namespaceId, serviceName);
        if (service == null) {
            ...
            //所属集群cluster不为空时，设置cluster的service，service中put一个cluster
            if (cluster != null) {
    ...
    public void addInstance(String namespaceId, String serviceName, boolean ephemeral, Instance... ips)
            throws NacosException {
        //根据ephemeral组装key是ephemeral还是persistent
        String key = KeyBuilder.buildInstanceListKey(namespaceId, serviceName, ephemeral);

        Service service = getService(namespaceId, serviceName);

        synchronized (service) {
            //根据ephemeral从ephemeralConsistencyService或者persistentConsistencyService中获取实例集合
            List<Instance> instanceList = addIpAddresses(service, ephemeral, ips);

            Instances instances = new Instances();
            instances.setInstanceList(instanceList);
            //根据ephemeral将实例集合放入对应的datum，并发布service改变事件，同时通知其他节点
    ...
    public List<Instance> updateIpAddresses(Service service, String action, boolean ephemeral, Instance... ips)
            throws NacosException {
        //根据namespaceId、name、ephemeral从对应的ephemeralConsistencyService、persistentConsistencyService获取datum
        Datum datum = consistencyService
                .get(KeyBuilder.buildInstanceListKey(service.getNamespaceId(), service.getName(), ephemeral));
        //获取当前service已经注册的实例信息
        ...
        if (datum != null && null != datum.value) {
            //如果当前service的实例对应的ip在currentInstances中，设置datum中对应的实例的healthy，lastBeat
        ...
        for (Instance instance : ips) {
            //如果实例的cluster在service中不存在，则初始化cluster，并放入service中
            //第一次注册是不存在的
    ...
    private void putServiceAndInit(Service service) throws NacosException {
        //将service放入serviceMap中
        putService(service);
        service = getService(service.getNamespaceId(), service.getName());
        //初始化service，创建心跳检查任务
        service.init();
        //将service放入listeners
        //ap:DistroConsistencyServiceImpl
        consistencyService
                .listen(KeyBuilder.buildInstanceListKey(service.getNamespaceId(), service.getName(), true), service);
        //cp:PersistentConsistencyServiceDelegateImpl
        consistencyService
